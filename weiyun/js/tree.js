...
//      div.style.zIndex=10;
        ...
        div.onmousedown=function(e){
			...
			document.onmousemove=function(e){
				...
				var divHeight=parseFloat(getComputedStyle(div).top);
				var divWidth=parseFloat(getComputedStyle(div).left);
				L=divWidth+l-div.offsetLeft;
				T=divHeight+t-div.offsetTop;
				for (var i = 0; i < divElement.length; i++) {
			...
			document.onmouseup=function(e){
				...
				content.removeChild(movePic);
				for (var i = getcheck().length-1; i >=0; i--) {
					getcheck()[i].style.left=L+'px';
					getcheck()[i].style.top=T+'px';
					getcheck()[i].className='file';
					getcheck()[i].checked=false;
				}
